...
const NavbarLS = ({ showSearchBar }) => {
  ...
  return (
    <nav className="ml-[280px] fixed top-0 left-0 right-0 m-5 mx-10 bg-white p-3 shadow-md rounded-md flex items-center justify-between z-50">
      ...
      {!showSearchBar && (
        <h1 className='text-xl font-semibold'>Welcome, {lembagaSosialData.name}!</h1>
      )}
      ...
      <ul className="flex items-center gap-5">
        ...
        <div className="flex items-center gap-2 relative" ref={dropdownRef}>
          ...
          <div className='flex items-center gap-3'>
            <div>
              <h1>{lembagaSosialData.name}</h1>
              <h1 className="text-xs">{lembagaSosialData.location}</h1>
            </div>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
